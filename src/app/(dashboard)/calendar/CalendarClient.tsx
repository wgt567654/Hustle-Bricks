...
export default function CalendarClient({
  ...
  initialClients,
  initialBusinessId,
  initialAvailTableMissing,
  initialBookingRequests,
  initialBookingTableMissing,
  initialUnavailDates,
  initialSchedulingSettings,
  initialDefaultCrewSize,
  initialSmartSchedulingEnabled,
}: {
  initialJobs: Job[];
  ...
  initialAvailTableMissing: boolean;
  initialBookingRequests: BookingRequest[];
  initialBookingTableMissing: boolean;
  initialUnavailDates: string[];
  initialSchedulingSettings: {
    unavailable_days: number[];
    day_hours: Record<string, { from: string; until: string }>;
  };
  initialDefaultCrewSize: number;
  initialSmartSchedulingEnabled: boolean;
}) {
  ...
  const [availTableMissing] = useState(initialAvailTableMissing);
  const [bookingRequests, setBookingRequests] = useState<BookingRequest[]>(initialBookingRequests);
  const [bookingTableMissing] = useState(initialBookingTableMissing);
  const [processingBooking, setProcessingBooking] = useState<string | null>(null);
  const [unavailDates, setUnavailDates] = useState<Set<string>>(() => new Set(initialUnavailDates));
  ...
  // Smart scheduling
  const [smartSchedulingEnabled] = useState(initialSmartSchedulingEnabled);
  ...
  const [defaultCrewSize] = useState(initialDefaultCrewSize);

  const year = currentDate.getFullYear();
  const month = currentDate.getMonth();

  useEffect(() => {
    // Fetch Google Calendar connection status (API route, not a Supabase query)
...
CREATE POLICY "owner_manage_availability" ON worker_availability
  FOR ALL USING (business_id IN (
    SELECT id FROM businesses WHERE owner_id = auth.uid()
          ...
          {team.length > 0 && !availTableMissing && (
            <Card className="rounded-2xl border-border shadow-sm overflow-hidden">
              <div className="p-4 border-b border-border/50">
