...
export default function PaymentsClient({
  ...
  initialJobs: JobWithPayment[];
  initialBusinessId: string | null;
  initialCurrency: string;
}) {
  const router = useRouter();
  const [jobs, setJobs] = useState<JobWithPayment[]>(initialJobs);
    ...
    <div className="flex flex-col gap-4 px-4 lg:px-8 py-4 max-w-xl mx-auto lg:max-w-none pb-32 lg:pb-8">
          ...
          <div className="relative z-10 flex flex-col gap-5">
            ...
            <div className="flex items-baseline gap-1">
              <span className="text-5xl font-extrabold tracking-tighter text-white leading-none">
                {formatCurrencyRounded(totalEarned, currency)}
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-white/15 rounded-2xl px-3 py-2.5 flex flex-col gap-0.5">
                ...
                <span className="text-xl font-extrabold text-white leading-none">{paidJobs.length}</span>
              </div>
              <div
              ...
              >
                ...
                <span className={`text-xl font-extrabold leading-none ${unpaidJobs.length > 0 ? "text-[#fbbf24]" : "text-white"}`}>
                  {unpaidJobs.length > 0 ? unpaidJobs.length : "✓ Clear"}
                </span>
              </div>
            </div>
          </div>
      ...
      {payModal && (
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-50 flex flex-col max-h-[80vh] max-w-xl mx-auto rounded-t-3xl bg-background shadow-2xl border-t border-border overflow-hidden" ref={swipePay.sheetRef} {...swipePay.sheetDragProps}>
