  ...
  useEffect(() => {
    async function load() {
      ...
      for (const msg of (messagesData ?? []) as LastMessage[]) {
        if (!latest[msg.team_member_id]) {
          latest[msg.team_member_id] = msg;
        }
        if (msg.sender_role === "employee" && !msg.is_read) {
          unread[msg.team_member_id] = (unread[msg.team_member_id] ?? 0) + 1;
        }
      }
      setLastMessages(latest);
      setUnreadCounts(unread);
      setLastBroadcast(broadcastData as Broadcast | null);
      ...
      setGroups(
        ((groupsData ?? []) as unknown as { id: string; name: string; type: string; team_group_members: { team_member_id: string }[] }[]).map((g) => ({
          id: g.id,
          name: g.name,
          type: g.type,
