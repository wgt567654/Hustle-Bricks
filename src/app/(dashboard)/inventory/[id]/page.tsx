...
export default function InventoryDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [presetCrews, setPresetCrews] = useState<PresetCrew[]>([]);
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!itemData) { router.push("/inventory"); return; }

      setItem(itemData as InventoryItem);
      setEditForm(itemData as InventoryItem);
      setAssignments((assignData as unknown as Assignment[]) ?? []);
      setUsageLogs((usageData as unknown as UsageLog[]) ?? []);
      setTeamMembers((tmData as TeamMember[]) ?? []);
      setPresetCrews(
        ((pcData ?? []) as unknown as { id: string; name: string; preset_crew_members: { team_member_id: string }[] }[]).map((pc) => ({
          id: pc.id,
          name: pc.name,
