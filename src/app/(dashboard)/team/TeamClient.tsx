...
export default function TeamClient({
  ...
  initialBusinessId,
  initialWorkload,
  initialMemberAvailability,
  initialMemberBlockedDates,
  initialMemberTerritories,
}: {
  initialMembers: TeamMember[];
  initialPendingMembers: TeamMember[];
  initialBusinessId: string | null;
  initialWorkload: Record<string, number>;
  initialMemberAvailability: Record<string, Record<number, { from: string; until: string }>>;
  initialMemberBlockedDates: Record<string, string[]>;
  initialMemberTerritories: Record<string, string[]>;
}) {
  ...
  const [workload] = useState<Record<string, number>>(initialWorkload);
  const [pendingMembers, setPendingMembers] = useState<TeamMember[]>(initialPendingMembers);
  ...
  const [memberAvailability, setMemberAvailability] = useState<Record<string, Record<number, { from: string; until: string }>>>(initialMemberAvailability);
  ...
  const [memberBlockedDates, setMemberBlockedDates] = useState<Record<string, Set<string>>>(
    () => Object.fromEntries(Object.entries(initialMemberBlockedDates).map(([k, v]) => [k, new Set(v)]))
  );
  ...
  const [memberTerritories, setMemberTerritories] = useState<Record<string, string[]>>(initialMemberTerritories);
  ...
  useEffect(() => {
    if (pendingMembers.length > 0 && window.location.search.includes("pending=true")) {
  ...
  return (
    <div className="flex flex-col gap-4 px-4 lg:px-8 py-4 max-w-xl mx-auto lg:max-w-none pb-32 lg:pb-8">
      ...
      {pendingMembers.length > 0 && (
        <div ref={pendingSectionRef} className={`flex flex-col gap-3 rounded-2xl transition-all duration-300 ${highlightPending ? "ring-2 ring-[var(--color-status-in-progress)] ring-offset-2 bg-status-in-progress/5 p-2" : ""}`}>
