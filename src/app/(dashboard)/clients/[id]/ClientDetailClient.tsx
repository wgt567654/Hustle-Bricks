...
export default function ClientDetailClient({
  ...
  initialConnectStatus: string;
  initialBookingRequests: BookingRequest[];
  initialBillingSub: BillingSub | null;
}) {
  const router = useRouter();
  const [client, setClient] = useState<ClientDetail | null>(initialClient);
  ...
  const [form, setForm] = useState({
    ...
    tag: initialClient.tag as Tag,
    notes: initialClient.notes ?? "",
    recurring_plan: (initialClient.recurring_plan as RecurringPlan) ?? "none",
  });
  ...
  const [connectStatus] = useState<string>(initialConnectStatus);
  const [activeBillingSub, setActiveBillingSub] = useState<BillingSub | null>(initialBillingSub);
  ...
  const [notesValue, setNotesValue] = useState(initialClient.notes ?? "");
