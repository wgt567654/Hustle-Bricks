...
// Mirrors the original `select("*")` row; nullable columns kept loose.
export type SettingsBusiness = {
  id: string;
  name: string;
  employee_access_code: string | null;
  slug: string | null;
...
type SettingsClientProps = {
  initialBusiness: SettingsBusiness;
  initialUserEmail: string | null;
  initialOwnerName: string;
  initialSchedulingSettings: { unavailable_days: number[]; day_hours: Record<number, { from: string; until: string }> } | null;
  initialCrewSize: number;
  initialCustomFields: SettingsCustomField[];
};
...
export default function SettingsClient({
  ...
  const [businessId] = useState<string | null>(business.id);
  const [userEmail] = useState<string | null>(initialUserEmail);
  const [saving, setSaving] = useState(false);
  const [loading] = useState(false);

  // Payment method fields
  const [venmoUsername, setVenmoUsername] = useState(business.venmo_username ?? "");
  const [cashappTag, setCashappTag] = useState(business.cashapp_tag ?? "");
  const [checkPayableTo, setCheckPayableTo] = useState(business.check_payable_to ?? business.name ?? "");
  const [editingPayments, setEditingPayments] = useState(false);
  const [savingPayments, setSavingPayments] = useState(false);

  // Service areas (weather alerts)
  const [serviceAreas, setServiceAreas] = useState<string[]>(() => {
    const areas = business.service_areas ?? [];
    return areas.length ? areas : business.city ? [business.city] : [];
  });
  ...
  // Invoice message
  const [invoiceMessage, setInvoiceMessage] = useState(business.invoice_message ?? "");
  ...
  const [accessCode, setAccessCode] = useState<string | null>(business.employee_access_code ?? null);
  const [generatingCode, setGeneratingCode] = useState(false);
  const [codeCopied, setCodeCopied] = useState(false);
  const [joinLinkCopied, setJoinLinkCopied] = useState(false);

  // Website booking link / slug
  const [slug, setSlug] = useState<string | null>(business.slug ?? null);
  ...
  const [connectStatus, setConnectStatus] = useState<"not_connected" | "pending" | "active">((business.stripe_connect_status as "not_connected" | "pending" | "active") ?? "not_connected");
  const [connectType, setConnectType] = useState<string | null>(business.stripe_connect_type ?? null);
  ...
  const [stripeSubscriptionId] = useState<string | null>(business.stripe_subscription_id ?? null);
  const [subscriptionStatus] = useState<string | null>(business.subscription_status ?? null);
  const [plan] = useState<string | null>(business.plan ?? null);
  const [openingPortal, setOpeningPortal] = useState(false);

  // Automations
  const [smsReminders, setSmsReminders] = useState(business.sms_reminders_enabled ?? false);
  const [smartScheduling, setSmartScheduling] = useState(business.smart_scheduling_enabled ?? false);
  const [reviewRequests, setReviewRequests] = useState(business.review_requests_enabled ?? false);
  const [leadNotify, setLeadNotify] = useState(business.lead_notify_enabled ?? false);
  const [followUpEnabled, setFollowUpEnabled] = useState(business.follow_up_enabled ?? false);
  const [rebookingEnabled, setRebookingEnabled] = useState(business.rebooking_enabled ?? false);
  const [autoInvoiceEnabled, setAutoInvoiceEnabled] = useState(business.auto_invoice_enabled ?? true);
  const [paymentRemindersEnabled, setPaymentRemindersEnabled] = useState(business.payment_reminders_enabled ?? true);
  ...
  // Pipeline velocity
  const [staleQuoteDays, setStaleQuoteDays] = useState(business.stale_quote_days != null ? String(business.stale_quote_days) : "7");
  ...
  // Scheduling settings
  const [unavailableDays, setUnavailableDays] = useState<number[]>(initialSchedulingSettings?.unavailable_days ?? [0, 6]);
  const [dayHours, setDayHours] = useState<Record<number, { from: string; until: string }>>(
    initialSchedulingSettings?.day_hours ?? {
  ...
  const [crewSize, setCrewSize] = useState(initialCrewSize);
  ...
  const [customFields, setCustomFields] = useState<CustomField[]>(initialCustomFields as CustomField[]);
  ...
  const [financingEnabled, setFinancingEnabled] = useState(business.financing_enabled ?? false);
  const [financingPartner, setFinancingPartner] = useState(business.financing_partner ?? "");
  const [financingUrl, setFinancingUrl] = useState(business.financing_url ?? "");
  const [financingMinAmount, setFinancingMinAmount] = useState(business.financing_min_amount != null ? String(business.financing_min_amount) : "500");
