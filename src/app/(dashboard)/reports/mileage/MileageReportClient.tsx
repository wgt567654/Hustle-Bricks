...
export default function MileageReportClient({
  ...
  initialRecords: MileageRecord[];
}) {
  const [members] = useState<TeamMember[]>(initialMembers);
  const [selectedId, setSelectedId] = useState<string>(initialSelectedId);
  const [records, setRecords] = useState<MileageRecord[]>(initialRecords);
  const [calculating, setCalculating] = useState(false);
  const [calcDate, setCalcDate] = useState(() => new Date().toISOString().split("T")[0]);
  const [expanded, setExpanded] = useState<string | null>(null);
  const [calcError, setCalcError] = useState<string | null>(null);

  // Skip the first records fetch — the server already provided the records
  // for the initially selected member.
