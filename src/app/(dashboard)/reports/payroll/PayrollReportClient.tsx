...
export default function PayrollReportClient({
  ...
  initialAssignedJobs: AssignedJob[];
  businessId: string | null;
}) {
  const router = useRouter();
  const [entries, setEntries] = useState<TimeEntry[]>(initialEntries);
  const [assignedJobs, setAssignedJobs] = useState<AssignedJob[]>(initialAssignedJobs);
  ...
  // Skip the very first fetch — the server already provided the initial
  // (month-range) dataset matching the default filter state.
