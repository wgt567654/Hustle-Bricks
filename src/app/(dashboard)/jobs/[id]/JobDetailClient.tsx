...
export default function JobDetailClient({
  ...
  initialSmsRemindersEnabled: boolean;
  notFound: boolean;
}) {
  const router = useRouter();
  const [job, setJob] = useState<Job | null>(initialJob);
  const [updating, setUpdating] = useState(false);
  const [notFound] = useState(initialNotFound);
  ...
  const [smsRemindersEnabled] = useState(initialSmsRemindersEnabled);

  // Fallback: hydrate owner name from the client if the server JWT claims
  // didn't include user metadata (matches the original getUser-based source).
  useEffect(() => {
    if (initialOwnerName) return;
  ...
  const [payAmount, setPayAmount] = useState(() => initialJob ? String(initialJob.total.toFixed(2)) : "");
  ...
  return (
    <div className="flex flex-col gap-4 px-4 lg:px-8 py-4 max-w-xl mx-auto lg:max-w-none pb-52 lg:pb-48">
      ...
      {job.status === "completed" && !payModalOpen && (
        <div className="fixed bottom-0 left-0 w-full z-50 bg-card border-t border-border px-4 pt-4" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 1rem)" }}>
