...
export default function JobsClient({
  ...
  initialTomorrowJobs: TomorrowJob[];
  initialReminderTemplates: Record<string, string>;
  initialTeamMembers: CrewMember[];
  initialAllPayments: { amount: number; paid_at: string }[];
  initialOutstandingCount: number;
  ...
  const [allPayments] = useState<{ amount: number; paid_at: string }[]>(initialAllPayments);
  const [revenueFilter, setRevenueFilter] = useState<"today" | "week" | "month" | "year">("week");
  const [outstandingAmount] = useState(initialOutstandingAmount);
  const [outstandingCount] = useState(initialOutstandingCount);
  const [statsLoaded] = useState(true);
  const [currency] = useState(initialCurrency);
  const [smsReminders] = useState(initialSmsReminders);
  const [tomorrowJobs] = useState<TomorrowJob[]>(initialTomorrowJobs);
  const [businessName] = useState(initialBusinessName);
  const [reminderTemplates] = useState<Record<string, string>>(initialReminderTemplates);
  const [selectedCrewMember, setSelectedCrewMember] = useState<string | null>(null);
  const [completedExpanded, setCompletedExpanded] = useState(false);
  const [customDateFrom, setCustomDateFrom] = useState("");
  const [customDateTo, setCustomDateTo] = useState("");
  const [teamMembers] = useState<CrewMember[]>(initialTeamMembers);
  ...
  const [hasServiceAreas] = useState(initialServiceAreas.length > 0);
  ...
  // Fallback: hydrate greeting name from the client if the server JWT
  // claims didn't include user metadata (mirrors UserGreeting behavior).
  useEffect(() => {
    if (initialFirstName) return;
  ...
  useEffect(() => {
    const allAreas = initialServiceAreas;
