...
export default async function InvoicePage({
  ...
  // Invoices are opened by anonymous customers; RLS on jobs is owner/member
  // scoped, so fetch with the service role — the link is keyed on the
  // unguessable job UUID (same trust model as the client portal).
    ...
    return (
      <div className="min-h-screen bg-background text-foreground flex items-center justify-center">
        <div className="text-center flex flex-col items-center gap-4">
          ...
          <p className="text-sm text-muted-foreground">This invoice link may be invalid or has expired.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 sm:px-6 sm:py-10">
        ...
        <div className="flex items-start justify-between mb-8">
          <div className="flex flex-col gap-1">
            ...
            {job.businesses?.address && (
              <p className="text-xs text-muted-foreground mt-0.5">{job.businesses.address}</p>
            )}
            {job.businesses?.website_url && (
              <a href={job.businesses.website_url} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline">
                {job.businesses.website_url.replace(/^https?:\/\//, "")}
              </a>
            )}
            <p className="text-sm text-muted-foreground font-medium mt-1">Invoice</p>
            <p className="text-xs text-muted-foreground font-mono">{refNum}</p>
          </div>
            ...
            >
              <span
                className={`w-2 h-2 rounded-full ${
                  isPaid ? "bg-status-completed" : "bg-status-in-progress"
                }`}
              />
              {isPaid ? "PAID" : "UNPAID"}
            </span>
        ...
        <div className="grid grid-cols-2 gap-8 mb-8">
          <div className="flex flex-col gap-1">
            ...
            <p className="font-bold text-foreground text-base">{job.clients?.name ?? "—"}</p>
            {job.clients?.address && (
              <p className="text-sm text-muted-foreground leading-relaxed">{job.clients.address}</p>
            )}
            {job.clients?.phone && (
          ...
          <div className="flex flex-col gap-1 items-end text-right">
            ...
            <p className="text-sm text-muted-foreground">
              <span className="font-semibold text-foreground">Invoice:</span> {refNum}
            </p>
            {job.scheduled_at && (
              <p className="text-sm text-muted-foreground">
          ...
          <table className="w-full min-w-[480px]">
            <thead>
              <tr className="border-b-2 border-border">
                <th className="text-left text-[10px] font-bold uppercase tracking-widest text-muted-foreground pb-3 pr-4">
                  Description
                </th>
                <th className="text-center text-[10px] font-bold uppercase tracking-widest text-muted-foreground pb-3 px-4 w-16">
                  Qty
                </th>
                <th className="text-right text-[10px] font-bold uppercase tracking-widest text-muted-foreground pb-3 px-4 w-28">
                  Unit Price
                </th>
                <th className="text-right text-[10px] font-bold uppercase tracking-widest text-muted-foreground pb-3 pl-4 w-28">
                  Total
                </th>
              </tr>
            </thead>
            <tbody>
              {job.job_line_items.map((item, i) => (
                ...
                >
                  ...
                  <td className="py-3.5 px-4 text-sm text-center text-muted-foreground">
                    {item.quantity}
                  </td>
                  <td className="py-3.5 px-4 text-sm text-right text-muted-foreground">
                    {formatCurrency(item.unit_price, currency)}
                  </td>
                  <td className="py-3.5 pl-4 text-sm text-right font-semibold text-foreground">
        ...
        <div className="flex justify-end mb-8">
          <div className="w-64 flex flex-col gap-2">
            <div className="flex items-center justify-between py-2 border-t border-border">
              ...
              <span className="text-sm font-semibold text-foreground">{formatCurrency(subtotal, currency)}</span>
            </div>
            <div className="flex items-center justify-between py-3 border-t-2 border-foreground">
              ...
              <span className="text-xl font-extrabold text-foreground">{formatCurrency(job.total, currency)}</span>
            </div>
          </div>
        </div>
        ...
        {isPaid && paidPayment && (
          <div className="mb-8 rounded-2xl bg-status-completed/10 border border-status-completed/20 shadow-card p-5 flex items-center gap-4">
            ...
            <div>
              <p className="font-bold text-status-completed text-sm">Payment received</p>
              <p className="text-sm text-muted-foreground">
        ...
        {/* Notes */}
        {job.notes && (
          <div className="mb-8 rounded-2xl bg-card border border-border shadow-card p-5">
        ...
        {job.signature_url && (
          <div className="mb-8 rounded-2xl bg-card border border-border shadow-card p-5">
            <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-3">Customer Signature</p>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
        ...
        {!isPaid && (
          <div className="mb-8 rounded-2xl border border-border bg-card shadow-card overflow-hidden">
            <div className="px-5 py-3 bg-muted/50 border-b border-border">
              <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">How to Pay</p>
            </div>
            <div className="flex flex-col divide-y divide-border/60">
              ...
              {job.businesses?.venmo_username && (
                ...
                >
                  ...
                  <div className="flex flex-col flex-1">
                    <span className="font-bold text-foreground text-sm">Pay with Venmo</span>
                    <span className="text-xs text-muted-foreground">@{job.businesses.venmo_username} · {formatCurrency(job.total, currency)}</span>
                  </div>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 text-muted-foreground group-hover:text-[#008CFF] transition-colors">
              ...
              {job.businesses?.cashapp_tag && (
                ...
                >
                  ...
                  <div className="flex flex-col flex-1">
                    <span className="font-bold text-foreground text-sm">Pay with Cash App</span>
                    <span className="text-xs text-muted-foreground">${job.businesses.cashapp_tag} · {formatCurrency(job.total, currency)}</span>
                  </div>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 text-muted-foreground group-hover:text-[#00C244] transition-colors">
                    <path fillRule="evenodd" d="M5.22 14.78a.75.75 0 001.06 0l7.22-7.22v5.69a.75.75 0 001.5 0v-7.5a.75.75 0 00-.75-.75h-7.5a.75.75 0 000 1.5h5.69l-7.22 7.22a.75.75 0 000 1.06z" clipRule="evenodd" />
                  </svg>
                </a>
              )}
              {job.businesses?.check_payable_to && (
                <div className="flex items-center gap-4 px-5 py-4">
                  ...
                  <div className="flex flex-col flex-1">
                    <span className="font-bold text-foreground text-sm">Pay by Check</span>
                    <span className="text-xs text-muted-foreground">Make payable to: <span className="font-semibold text-foreground">{job.businesses?.check_payable_to}</span></span>
                  </div>
                </div>
              )}
            </div>
        ...
        {!isPaid &&
          ...
          <div className="mb-8 rounded-2xl bg-primary/5 border border-primary/20 shadow-card overflow-hidden">
            <div className="px-5 py-3 bg-primary/10 border-b border-primary/20">
              <p className="text-xs font-bold uppercase tracking-widest text-primary">Pay Over Time</p>
            </div>
            <div className="px-5 py-4 flex flex-col gap-3">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 py-3 rounded-full bg-primary text-white font-bold text-sm text-center hover:bg-primary/90 active:scale-95 transition-all"
              >
              ...
              {job.businesses.contact_email && (
                ...
                >
                  ...
                  <div className="flex flex-col flex-1">
                    ...
                    <span className="text-xs text-muted-foreground">{job.businesses.contact_email}</span>
                  </div>
              ...
              {job.businesses.contact_phone && (
                ...
                >
                  ...
                  <div className="flex flex-col flex-1">
                    ...
                    <span className="text-xs text-muted-foreground">{job.businesses.contact_phone}</span>
                  </div>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 text-muted-foreground group-hover:text-foreground transition-colors">
                    <path fillRule="evenodd" d="M5.22 14.78a.75.75 0 001.06 0l7.22-7.22v5.69a.75.75 0 001.5 0v-7.5a.75.75 0 00-.75-.75h-7.5a.75.75 0 000 1.5h5.69l-7.22 7.22a.75.75 0 000 1.06z" clipRule="evenodd" />
                  </svg>
        ...
        {job.businesses?.invoice_message && (
          <div className="mb-8 rounded-2xl border border-border bg-card shadow-card px-5 py-4">
            <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-2">A Note from {job.businesses.name}</p>
            <p className="text-sm text-foreground leading-relaxed">{job.businesses.invoice_message}</p>
          </div>
        )}

        {/* Terms & Conditions */}
        {job.businesses?.terms_and_conditions && (
          <details className="mb-8 rounded-2xl border border-border bg-card shadow-card overflow-hidden group">
            <summary className="px-5 py-4 flex items-center justify-between cursor-pointer bg-muted/50 hover:bg-muted transition-colors list-none">
              <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Terms &amp; Conditions</p>
            ...
            <div className="px-5 py-4 border-t border-border/60">
              <p className="text-xs text-muted-foreground leading-relaxed whitespace-pre-wrap">{job.businesses.terms_and_conditions}</p>
            </div>
          </details>
        )}

        {/* Footer */}
        <div className="border-t border-border/60 pt-6 mt-4 text-center">
          <p className="text-xs text-muted-foreground">
            Powered by{" "}
            <span className="font-bold text-muted-foreground">HustleBricks</span>
          </p>
        </div>
      </div>
    </div>
