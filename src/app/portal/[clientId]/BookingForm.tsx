...
export function BookingForm({
  ...
  // eslint-disable-next-line react-hooks/set-state-in-effect -- pre-existing pattern; styling-only migration (task 3.4)
  ...
  if (submitted) {
    return (
      <div className="bg-status-completed/10 border border-status-completed/25 rounded-2xl p-6 flex flex-col items-center gap-3 text-center">
        <div className="flex size-14 items-center justify-center rounded-full bg-status-completed/15">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-7 h-7 text-status-completed">
            <path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm13.36-1.814a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z" clipRule="evenodd" />
          </svg>
        </div>
        <div>
          <p className="font-extrabold text-status-completed text-base">Request sent!</p>
  ...
  if (isPending && !open) {
    return (
      <div className="bg-status-in-progress/10 border border-status-in-progress/25 rounded-2xl p-5 flex items-center gap-4">
        <div className="flex size-11 shrink-0 items-center justify-center rounded-full bg-status-in-progress/15">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 text-status-in-progress">
            <path fillRule="evenodd" d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25zM12.75 6a.75.75 0 00-1.5 0v6c0 .414.336.75.75.75h4.5a.75.75 0 000-1.5h-3.75V6z" clipRule="evenodd" />
          </svg>
        </div>
        <div className="flex-1 min-w-0">
          <p className="font-extrabold text-status-in-progress text-sm">Appointment request pending</p>
          <p className="text-xs text-foreground mt-0.5">
            {formatDateLabel(latestRequest!.requested_date)} · {formatSlot(latestRequest!.requested_time)}
          </p>
          <p className="text-xs text-muted-foreground mt-1">We&apos;ll confirm your time soon.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-card rounded-2xl border border-border shadow-card overflow-hidden">
      ...
      {isDeclined && (
        <div className="px-5 pt-5">
          <div className="bg-destructive/10 border border-destructive/25 rounded-xl p-4 flex items-start gap-3 mb-4">
            <div className="flex size-9 shrink-0 items-center justify-center rounded-full bg-destructive/15 mt-0.5">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-destructive">
                <path fillRule="evenodd" d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25zm-1.72 6.97a.75.75 0 10-1.06 1.06L10.94 12l-1.72 1.72a.75.75 0 101.06 1.06L12 13.06l1.72 1.72a.75.75 0 101.06-1.06L13.06 12l1.72-1.72a.75.75 0 10-1.06-1.06L12 10.94l-1.72-1.72z" clipRule="evenodd" />
              </svg>
            </div>
            <div>
              <p className="font-extrabold text-destructive text-sm">That time isn&apos;t available</p>
              <p className="text-xs text-foreground mt-0.5">
                {formatDateLabel(latestRequest!.requested_date)} · {formatSlot(latestRequest!.requested_time)}
              </p>
              <p className="text-xs text-muted-foreground mt-1">Please pick a different date or time below.</p>
            </div>
          </div>
        </div>
      )}
      ...
      {!isDeclined && (
        <button
          onClick={() => setOpen((o) => !o)}
          className="w-full flex items-center gap-4 px-5 py-4 hover:bg-muted/50 transition-colors group"
        >
          ...
          <div className="flex flex-col flex-1 text-left">
            <span className="font-extrabold text-foreground text-sm">Schedule a Service</span>
            <span className="text-xs text-muted-foreground">Request an appointment time</span>
          </div>
          <svg
      ...
      {open && (
        <div className="border-t border-border px-5 pt-5 pb-6 flex flex-col gap-5">
                ...
                return (
                  <button
                    ...
                    className={`relative flex items-center justify-center h-9 rounded-xl text-sm font-bold transition-all active:scale-95 ${
                      isBlocked
                        ? "bg-destructive/10 text-destructive/40 cursor-not-allowed line-through"
                        : isSelected
                  ...
                  {timeSlots.map((slot) => {
                    ...
                    return (
                      ...
                      >
                        {formatSlot(slot)}
                        {isLow && !isSelected && (
                          <span className="text-[9px] font-bold text-caution">{cap} left</span>
                        )}
                        {isFull && (
                          <span className="text-[9px] font-bold text-muted-foreground no-underline" style={{ textDecoration: "none" }}>Full</span>
                        )}
                      </button>
                    );
                  })}
          ...
          {selectedDate && selectedTime && (
            <div className="flex flex-col gap-2">
              ...
              <textarea
                ...
                rows={3}
                className="w-full rounded-xl border border-border bg-muted/50 px-3 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring/30 resize-none"
              />
            </div>
          )}
          ...
          {selectedDate && selectedTime && (
            <div className="flex flex-col gap-3">
              ...
              <button
                ...
                disabled={submitting}
                className="w-full py-4 rounded-full bg-primary text-primary-foreground font-extrabold text-sm hover:bg-primary/90 disabled:opacity-50 active:scale-[0.98] transition-all shadow-lg shadow-primary/20"
              >
          ...
          {!selectedDate && (
            <p className="text-xs text-muted-foreground text-center">Select a date above to see available times</p>
          )}
        </div>
      )}
    </div>
