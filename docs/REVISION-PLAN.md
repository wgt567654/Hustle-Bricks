...
4. ~~**Skill-aware dispatch + owner certification + multi-worker fix**~~ ✅ DONE 2026-07-20: member_service_capabilities table (source owner|training); owner "Certified Services" toggles in Team editor; findBestMember filters by qualification (adoption-friendly fallback: uncertified service → everyone qualifies); assign modal shows Qualified/Not-certified (non-blocking, owner confirms). Multi-worker: widened employee job list+detail to job_crew membership (not just primary) + employee_crew_access.sql RLS via SECURITY DEFINER helpers (fixed jobs↔job_crew recursion). Verified live all three.
...
## 10. STRATEGIC PIVOT + Sprint 4 — HQ / Micro-PE data play ✅ DEPLOYED 2026-07-20

Revenue model shift (founder, 2026-07-20): SaaS subscription is a weak moat in the vibe-coding era. New thesis — HustleBricks is **micro-private-equity**: a free/cheap OS drives adoption; the platform observes which micro-businesses & salespeople are best (data = the moat); monetize via (near→far) group-buying/supplies, embedded insurance (we underwrite better because we see operations), and micro-roll-ups / "HustleBricks Select" collective. Toast/Square/Shopify Capital playbook ("software is the sensor, not the product"). Investor framing: full-size PE rolls up HVAC; we're micro-level PE with a data moat; HQ screen = live deal flow; 10x TAM story.
Guardrail: monetize by OFFERING observed businesses value (supplies/insurance/capital/leads), never "we surveil & sell data." Needs a real ToS pass before onboarding non-family businesses (current privacy/terms are boilerplate).

**Sprint 4 BUILT + VERIFIED + DEPLOYED** (commit 6d82d34, live on hustlebricks.ai): `/hq` gated by HQ_ADMIN_EMAILS (you + Wil; local also has hb.e2e.test2 for testing) — cross-business deal-flow dashboard via service role. **Hustle Score** = mean of 5 transparent sub-scores (Growth/Grind/Close/Crew/Collect; formulas on-page). Business leaderboard (Acquisition Watchlist badge on top decile), cross-platform salesperson leaderboard (Top Closer), per-business AI Scout Report (Claude PE-memo; disabled w/o ANTHROPIC_API_KEY — graceful). Solo tier now **Free** (landing + onboarding skips Stripe → subscription_status='active', plan='solo'; Team/Business Stripe intact). Verified live w/ 5 seeded demo businesses (Shine Squad #1 score 78 +175% growth; CFoam #2). Gate redirects non-admins (prod /hq → /login confirmed).
Sprint 4 originally-queued items (drive-time buffer, AI dispatcher) → now Sprint 5.
